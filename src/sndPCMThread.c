...
int setparams_stream(snd_pcm_t* handle, snd_pcm_hw_params_t* params, const char* id, gpointer data) {
    ...
    if (handle == Phandle)
        err = snd_pcm_hw_params_set_channels(handle, params, da->settings.channels);
    else
        err = snd_pcm_hw_params_set_channels(handle, params, da->settings.channels);
...
int setparams_bufsize(snd_pcm_t* handle, snd_pcm_hw_params_t* params, snd_pcm_hw_params_t* tparams, const char* id,
                      gpointer data)
...
int setparams(snd_pcm_t* phandle, snd_pcm_t* chandle, gpointer data) {
    ...
    if (ok)
        printf("Pause Ok read\n");
    else
        printf("NO read\n");
    ok = snd_pcm_hw_params_can_pause(c_params);
    if (ok)
        printf("Pause Ok write\n");
    else
        printf("NO write\n");
...
int mic_poll_loop(snd_pcm_t* chandle, snd_pcm_t* phandle, VApp* da) {
    ...
    while (da->status.open) {
        if (!init) {
            ...
            if (cerr < 0) {
                if (snd_pcm_state(chandle) == SND_PCM_STATE_XRUN || snd_pcm_state(chandle) == SND_PCM_STATE_SUSPENDED) {
        ...
        while (cperiod > 0 && da->status.open) {
            ...
            if (perr < 0) {
                if (xrun_recovery(phandle, perr) < 0) {
                    printf("Write error: %s\n", snd_strerror(cerr));
                    exit(EXIT_FAILURE);
                }
                init = 1;
...
int file_poll_loop(snd_pcm_t* phandle, VApp* da) {
    int i;
    struct pollfd* ufds;
    double phase = 0;

    int16_t* pptr;
    int perr, count, pperiod, init;
    ...
    if ((perr = snd_pcm_poll_descriptors(phandle, ufds, count)) < 0) {
        printf("Unable to obtain poll descriptors for playback: %s\n", snd_strerror(perr));
        return perr;
    }
    ...
    while (da->status.open) {
        if (!init) {
            perr = wait_for_poll(phandle, ufds, count);
            if (perr < 0) {
                if (snd_pcm_state(phandle) == SND_PCM_STATE_XRUN || snd_pcm_state(phandle) == SND_PCM_STATE_SUSPENDED) {
                    perr = snd_pcm_state(phandle) == SND_PCM_STATE_XRUN ? -EPIPE : -ESTRPIPE;
                    printf("1>>>>>>>\n");
                    if (xrun_recovery(phandle, perr) < 0) {
                        printf("Write error: %s\n", snd_strerror(perr));
                        exit(EXIT_FAILURE);
        ...
        pptr = da->soundWrite.samples + writeCount;
        pperiod = da->soundRead.periodsize;
        while (pperiod > 0 && da->status.open) {

            perr = snd_pcm_writei(phandle, pptr, pperiod);
            if (perr < 0) {
                if (xrun_recovery(phandle, perr) < 0) {
                    printf("Write error: %s\n", snd_strerror(perr));
                    exit(EXIT_FAILURE);
...
err:
    ...
    if (!da->status.ref) {
        ...
        printf("Error soundinit() -> status.ref > 0 ref= %d\n", da->status.ref);
    fflush(stdout);
